...
KNOWN_MODEL_ID_PREFIXES = ("claude-", "gpt-", "gemini-")
...
class CLinkRequest(BaseModel):
    ...
    def _validate_model(cls, value: Any) -> str | None:
        ...
        if len(model) > MAX_MODEL_CHARS:
            raise ValueError(f"model must be {MAX_MODEL_CHARS} characters or fewer")
        if model.startswith("-"):
...
class CLinkTool(SimpleTool):
    ...
    def get_input_schema(self) -> dict[str, Any]:
        ...
        properties = {
            ...
            "model": {
                ...
                "description": (
                    "Optional CLI model name. Use native to suppress clink model injection and let the CLI "
                    "default apply."
                ),
            },
    ...
    def _raise_tool_error(self, message: str, metadata: dict[str, Any] | None = None) -> None:
        ...
        raise ToolExecutionError(error_output.model_dump_json(exclude_none=True))
    ...
    def _model_mismatch(self, effective_model: str | None, model_used: Any) -> bool | str | None:
        if not effective_model or not isinstance(model_used, str) or not model_used:
            return None

        expected = effective_model.lower()
        used = model_used.lower()
        if used == expected or used.startswith(expected + "-"):
            return False

        family = MODEL_ALIAS_FAMILIES.get(expected)
        if family:
            return not (used == family or used.startswith(family + "-"))

        if expected.startswith(KNOWN_MODEL_ID_PREFIXES):
            return True
