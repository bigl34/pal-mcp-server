...
class TestConsensusTool:
    ...
    def test_host_model_policy_skips_codex_gpt56_sol_pro(self, monkeypatch):
        """Codex consensus should skip GPT-5.6 Sol Pro and keep the Fable seat."""
        monkeypatch.setenv("PAL_ASSISTANT_FRONTEND", "codex")
        tool = ConsensusTool()

        filtered, skipped, frontend = tool._apply_host_model_skip_policy(
            [
                {"model": "gpt-5.6-sol-pro", "stance": "neutral"},
                {"model": "pro", "stance": "neutral"},
                {"model": "grok", "stance": "neutral"},
                {"model": "deepseek", "stance": "neutral"},
                {"model": "fable-latest", "stance": "neutral"},
            ]
        )

        assert frontend == "codex"
        assert [model["model"] for model in filtered] == ["pro", "grok", "deepseek", "fable-latest"]
        assert [model["model"] for model in skipped] == ["gpt-5.6-sol-pro"]

    def test_host_model_policy_skips_all_new_and_compatibility_aliases(self, monkeypatch):
        monkeypatch.setenv("PAL_ASSISTANT_FRONTEND", "codex")
        tool = ConsensusTool()
        aliases = [
            "gpt-5.6-sol-pro",
        ...
        filtered, skipped, _ = tool._apply_host_model_skip_policy(
            [{"model": model, "stance": "neutral"} for model in aliases]
            + [{"model": "fable-latest", "stance": "neutral"}]
        )

        assert [model["model"] for model in filtered] == ["fable-latest"]
        assert [model["model"] for model in skipped] == aliases

    def test_host_model_policy_uses_capability_frontend_dedup(self, monkeypatch):
        monkeypatch.setenv("PAL_ASSISTANT_FRONTEND", "codex")
        tool = ConsensusTool()
        provider = Mock()
        provider.get_capabilities.return_value = Mock(host_dedup_frontends=["codex"])
        tool.get_model_provider = Mock(return_value=provider)

        filtered, skipped, _ = tool._apply_host_model_skip_policy(
            [{"model": "arbitrary-logical-alias", "stance": "neutral"}]
        )

        assert filtered == []
        assert [model["model"] for model in skipped] == ["arbitrary-logical-alias"]

    def test_host_model_policy_resolution_failure_falls_back_to_aliases(self, monkeypatch):
        monkeypatch.setenv("PAL_ASSISTANT_FRONTEND", "codex")
        tool = ConsensusTool()
        tool.get_model_provider = Mock(side_effect=RuntimeError("registry unavailable"))

        filtered, skipped, _ = tool._apply_host_model_skip_policy([{"model": "gpt-5.5-pro", "stance": "neutral"}])

        tool.get_model_provider.assert_called_once_with("gpt-5.5-pro")
        assert filtered == []
        assert [model["model"] for model in skipped] == ["gpt-5.5-pro"]

    def test_host_model_policy_skips_claude_fable_latest(self, monkeypatch):
        """Claude Code consensus should skip Fable Latest and keep GPT-5.5 Pro."""
        ...
        assert frontend == "claude"
        assert [model["model"] for model in filtered] == ["gpt-5.5-pro", "pro", "grok", "deepseek"]
        assert [model["model"] for model in skipped] == ["fable-latest"]

    def test_host_model_policy_leaves_unknown_frontend_unchanged(self, monkeypatch):
        """Unknown clients should keep the full base panel."""
        ...
        filtered, skipped, frontend = tool._apply_host_model_skip_policy(models)

        assert frontend == "unknown"
    ...
    async def test_consult_model_preserves_requested_logical_and_upstream_names(self):
        ...
        provider.generate_content.return_value = Mock(
            ...
            metadata={"provider_model_name": "gpt-5.6-sol"},
        )
        tool.get_model_provider = Mock(return_value=provider)
        tool.validate_and_correct_temperature = Mock(return_value=(None, []))
        request = Mock(relevant_files=None, images=None)
        ...
        assert result["metadata"] == {
            "provider": "openai",
            "requested_model_name": "gpt-5.5-pro",
            "model_name": "gpt-5.6-sol-pro",
