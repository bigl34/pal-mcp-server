...
def test_claude_parser_extracts_result_and_metadata():
    ...
    assert parsed.metadata["is_error_explicit"] is True


def test_claude_parser_tolerates_stdout_noise_before_json_payload():
    parser = ClaudeJSONParser()
    stdout = "Warning: checking subscription state\n" + _build_success_payload()
...
def test_claude_parser_tolerates_bracketed_stdout_noise_before_json_payload():
    parser = ClaudeJSONParser()
    stdout = "[WARN] checking subscription state\n" + _build_success_payload()
...
def test_claude_parser_tolerates_braced_stdout_noise_before_json_payload():
    parser = ClaudeJSONParser()
    stdout = "Warning: retry metadata {not json}\n" + _build_success_payload()
...
def test_claude_parser_tolerates_bracketed_stdout_noise_before_json_array_payload():
    parser = ClaudeJSONParser()
    events = [
...
def test_claude_parser_rejects_trailing_non_whitespace_after_json_payload():
    parser = ClaudeJSONParser()
    stdout = _build_success_payload() + "\nextra"
...
@pytest.mark.parametrize(
    ("payload", "expected_is_error", "expected_explicit"),
    [
        ({"type": "result", "result": "42"}, False, False),
        ({"type": "result", "is_error": "false", "result": "42"}, True, False),
    ],
)
def test_claude_parser_tracks_is_error_explicitness(payload, expected_is_error, expected_explicit):
    parser = ClaudeJSONParser()
    stdout = json.dumps(payload)

    parsed = parser.parse(stdout=stdout, stderr="")

    assert parsed.metadata["is_error"] is expected_is_error
    assert parsed.metadata["is_error_explicit"] is expected_explicit
