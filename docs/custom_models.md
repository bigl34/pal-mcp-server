...
Native provider registries can also define a logical PAL profile whose public
name differs from the model sent over the wire. This is useful when one
upstream model supports several stable operating modes:
...
PAL uses `model_name` as the canonical identity in responses, logs, aliases,
and consensus metadata. It sends `provider_model_name` to the provider and
reports that upstream identity separately as `provider_model_name` metadata.
When the field is omitted, the canonical name is also the wire name.

`default_reasoning_mode` accepts `standard` or `pro`. PAL sends the configured
mode on OpenAI Responses requests and omits it only when unset.
`default_reasoning_effort` is deliberately a provider-dependent pass-through,
so values such as `max` are preserved rather than normalized through a global
enum. `host_dedup_frontends` names frontends that should skip the profile as a
same-host consensus seat; resolution failures fall back to PAL's compatibility
rules rather than dropping unrelated models.
...
**Field explanations:**
- `model_name`: PAL's canonical logical identifier (OpenRouter format like `vendor/model`, a local name like `llama3.2`, or a stable native-provider profile name)
- `provider_model_name`: Optional upstream wire identifier when it differs from `model_name`
