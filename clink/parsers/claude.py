...
class ClaudeJSONParser(BaseParser):
    ...
    def parse(self, stdout: str, stderr: str) -> ParsedCLIResponse:
        ...
        try:
            ...
            loaded = self._load_after_stdout_noise(stdout, exc)
    ...
    def _build_metadata(self, payload: dict[str, Any], stderr: str) -> dict[str, Any]:
        ...
        metadata: dict[str, Any] = {
            ...
            "is_error_explicit": isinstance(is_error_value, bool),
        }
    ...
    def _load_after_stdout_noise(self, stdout: str, original_error: json.JSONDecodeError) -> Any:
        decoder = json.JSONDecoder()
        last_error: json.JSONDecodeError = original_error
        for json_start, char in enumerate(stdout):
            if char not in "{[":
                continue
            try:
                loaded, end = decoder.raw_decode(stdout, idx=json_start)
            except json.JSONDecodeError as exc:
                last_error = exc
                continue
            if stdout[end:].strip():
                last_error = json.JSONDecodeError("Extra data", stdout, end)
                continue
            return loaded

        raise ParserError(f"Failed to decode Claude CLI JSON output: {last_error}") from last_error
