...
class OpenAICompatibleProvider(ModelProvider):
    ...
    def _ensure_model_allowed(
        ...
        if self.allowed_models is not None:
            ...
            provider_model = (capabilities.provider_model_name or canonical_name).lower()

            if not {requested, canonical, provider_model}.intersection(self.allowed_models):
                allowed = False
    ...
    def _allow_openrouter_non_zdr(self, model_name: Optional[str]) -> bool:
    ...
    def _to_responses_content(content, role: str) -> list:
        """Convert chat-style message content into /v1/responses content parts.

        The /v1/responses endpoint uses input_text / input_image parts (and
        output_text for assistant turns), not the chat-style {type:"text"} /
        {type:"image_url"} shape. `content` may be a plain string or a list
        of chat-style parts — the multimodal shape PAL builds when images are
        attached. Wrapping a list inside a single input_text.text field (the
        previous behaviour) made /v1/responses reject the request.
        """
        ...
        if isinstance(content, list):
            ...
            for part in content:
                if not isinstance(part, dict):
                    converted_parts.append({"type": text_type, "text": str(part)})
                    continue
                part_type = part.get("type")
                if part_type in ("text", "input_text", "output_text"):
                    converted_parts.append({"type": text_type, "text": part.get("text", "")})
                elif part_type in ("image_url", "input_image"):
                    # Chat shape nests the URL under image_url.url; the
                    # Responses API takes image_url as a bare string.
                    raw_image = part.get("image_url")
                    if isinstance(raw_image, dict):
                        image_ref = raw_image.get("url", "")
                    else:
                        image_ref = raw_image or ""
                    converted_parts.append({"type": "input_image", "image_url": image_ref})
                else:
                    # Unknown part type — pass through rather than drop it.
                    converted_parts.append(part)
            return converted_parts

        # None or any other type — coerce to a single text part.
        coerced_text = "" if content is None else str(content)
        return [{"type": text_type, "text": coerced_text}]

    def _await_responses_completion(self, response):
        """Poll a backgrounded /v1/responses run until it reaches a terminal state.

        background=True returns immediately with status "queued" or
        "in_progress". The run is fetched with responses.retrieve(id) on a
        2s -> x1.3 -> 10s-cap backoff, up to a 30-minute ceiling. The terminal
        response is returned on "completed"; any other terminal status
        (failed / cancelled / expired / incomplete) or a timeout raises, so a
        stuck run surfaces as an error and is never silently orphaned.

        A synchronous (non-background) response is already terminal and is
        returned unchanged.
        """
        ...
        if status not in in_progress_statuses:
            if isinstance(status, str) and status != "completed":
                raise RuntimeError(
                    f"/v1/responses run ended with status '{status}', not 'completed'"
                    f"{self._response_failure_details(response)}"
                )
            return response

        response_id = getattr(response, "id", None)
        if not response_id:
            raise RuntimeError("backgrounded /v1/responses run returned no id to poll")

        poll_delay = 2.0
        max_poll_delay = 10.0
        deadline = time.monotonic() + 30 * 60  # 30-minute ceiling

        while status in in_progress_statuses:
            if time.monotonic() >= deadline:
                raise RuntimeError(
                    f"/v1/responses run {response_id} did not finish within 30 minutes (last status: {status})"
                )
            time.sleep(poll_delay)
            poll_delay = min(poll_delay * 1.3, max_poll_delay)
            response = self.client.responses.retrieve(response_id)
            status = getattr(response, "status", None)

        if status != "completed":
            raise RuntimeError(
                f"/v1/responses run {response_id} ended with status '{status}', not 'completed'"
                f"{self._response_failure_details(response)}"
            )
        return response

    @staticmethod
    def _response_failure_details(response) -> str:
        """Return provider-supplied failure details for a terminal Responses run."""
        ...
        if error:
            ...
            if code or message:
                ...
                details.append(f"response.error ({', '.join(parts)})")
            else:
                details.append(f"response.error={error!r}")

        incomplete_details = getattr(response, "incomplete_details", None)
        if incomplete_details:
            details.append(f"incomplete_details={incomplete_details!r}")
    ...
    def _generate_with_responses_endpoint(
        ...
        **kwargs,
        ...
        for message in messages:
            ...
            if role == "system":
                # For o3-pro, system content is folded into a user turn
                # rather than prefixed with "System:" — avoids policy issues.
        ...
        if capabilities and capabilities.default_reasoning_mode:
            reasoning["mode"] = capabilities.default_reasoning_mode

        wire_model = provider_model_name or model_name
        completion_params = {
            "model": wire_model,
            "input": input_messages,
            "reasoning": reasoning,
        }
        completion_params.update(self._provider_request_options(model_name))

        provider_type = self.get_provider_type()

        # Only include store/background for providers that support OpenAI-style
        # stored response retrieval. OpenRouter rejects store:true; OpenCode Zen
        # accepts background creation but does not expose GET /responses/{id}.
        if provider_type not in [ProviderType.OPENROUTER, ProviderType.ZEN]:
            completion_params["store"] = True
            # background=True makes a large reasoning run return immediately
            # with a queued id we poll, instead of holding a sync connection
            # open — /v1/responses sync calls disconnect at ~180s. It
            # requires store=True, so it is set in the same branch.
            completion_params["background"] = True
        elif provider_type == ProviderType.ZEN:
            logging.debug(f"Omitting 'store' and 'background' parameters for Zen provider (model: {model_name})")
        else:
            logging.debug(f"Omitting 'store' parameter for OpenRouter provider (model: {model_name})")

        # Add max tokens if specified. The /v1/responses endpoint uses
        # max_output_tokens; max_completion_tokens was deprecated and is now
        # rejected ("Unsupported parameter: 'max_completion_tokens'").
        ...
        def _attempt() -> ModelResponse:
            ...
            return ModelResponse(
                ...
                metadata={
                    "model": getattr(response, "model", wire_model),
    ...
    def generate_content(
        ...
        **kwargs,
        ...
        routing_capabilities = capabilities or self.get_all_model_capabilities().get(resolved_model)
        wire_model = (
            routing_capabilities.provider_model_name
            if routing_capabilities and routing_capabilities.provider_model_name
            else resolved_model
        )
        ...
        completion_params.update(self._provider_request_options(resolved_model))
        ...
        if routing_capabilities is not None:
            use_responses_api = getattr(routing_capabilities, "use_openai_response_api", False)
    ...
    def _is_error_retryable(self, error: Exception) -> bool:
        ...
        if "429" in error_str:
            ...
            if error_type == "tokens":
                ...
                # Hard quota / billing-cap 429s will NOT clear within the retry
                # window (1+3+5+8s), so retrying only turns a definitive failure
                # into a multi-second hang. Fail fast on attempt 1 instead.
                logging.debug(f"Non-retryable 429: quota/billing exhausted (type={error_type}, code={error_code})")
                return False
            else:
