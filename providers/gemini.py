...
class GeminiModelProvider(RegistryBackedProviderMixin, ModelProvider):
    ...
    def client(self):
        ...
        if self._client is None:
            ...
            if self._timeout_override is not None:
                http_options_kwargs["timeout"] = self._to_gemini_timeout_ms(self._timeout_override)
    ...
    @staticmethod
    def _to_gemini_timeout_ms(timeout_seconds: float) -> int:
        """Convert PAL's second-based timeout env vars to google-genai milliseconds."""
        return max(int(timeout_seconds * 1000), 10_000)
