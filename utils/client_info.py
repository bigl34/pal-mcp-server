...
FRONTEND_ENV_VAR = "PAL_ASSISTANT_FRONTEND"

# Canonical frontend identifiers used for host-specific policy.
# Order matters: check more specific strings before broad ones.
CLIENT_FRONTEND_MAPPINGS = (
    ("codex", "codex"),
    ("claude-code", "claude"),
    ("claude", "claude"),
    ("anthropic", "claude"),
...
def _normalize_client_frontend(value: str | None) -> str | None:
    """Return a canonical frontend identifier for a raw env/client name."""
    if not value:
        return None
    ...
    for key, frontend in CLIENT_FRONTEND_MAPPINGS:
...
def get_current_client_frontend(client_name: str | None = None) -> str:
    """
    Identify the current frontend for host-specific behavior.

    Explicit environment wiring wins over clientInfo because wrappers can set it
    even when the MCP client does not expose clientInfo before a tool call.
    """
    env_frontend = _normalize_client_frontend(os.environ.get(FRONTEND_ENV_VAR))
    if env_frontend:
        return env_frontend

    raw_frontend = _normalize_client_frontend(client_name)
    if raw_frontend:
        return raw_frontend

    cached_info = get_cached_client_info()
    if cached_info:
        cached_frontend = cached_info.get("frontend")
        if cached_frontend:
            return cached_frontend

        cached_name = cached_info.get("name")
        normalized = _normalize_client_frontend(cached_name)
        if normalized:
            return normalized
...
def get_client_info_from_context(server: Any) -> Optional[dict[str, Any]]:
    ...
    try:
        ...
        result["frontend"] = get_current_client_frontend(raw_name)
