...
def configure_providers():
    ...
    if zen_key and zen_key != "your_zen_api_key_here":
        ...
        has_native_apis = True
        has_zen = True
        logger.info("OpenCode Zen API key found - curated Zen models available")
    ...
    if not valid_providers:
        raise ValueError(
            ...
            "- ZEN_API_KEY for OpenCode Zen models\n"
    ...
    priority_info = []
    if has_native_apis:
        priority_info.append("Native APIs (Gemini, OpenAI, X.AI, DIAL, Zen)")
    if has_custom:
    ...
    if IS_AUTO_MODE:
        ...
        if not available_models:
            logger.error(
                ...
                "Please check your provider-specific *_ALLOWED_MODELS settings."
            )
            raise ValueError(
...
async def handle_call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
    ...
    # Cache clientInfo on tool calls too. Some clients/runners call tools
    # without first listing tools, but host policy still needs frontend info.
    try:
        from utils.client_info import get_client_info_from_context

        get_client_info_from_context(server)
    except Exception as e:
        logger.debug(f"Could not cache client info during tool call: {e}")
